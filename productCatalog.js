...
function processSearchByType(searchType){
	console.log("processSearchByType")
    api.searchProductsByType(searchType)
		.then(function(val){
	    	return val;
    	}).then(function(val){
			updateExaminedText({price:'',id:'',type:''});
        	updateTable('similarTable',val);
    	}).catch(function(val){
        	alert("Error::ProcessSearchByType: "+val);
    	});
}
...
function processSearchByPrice(searchPrice){
    api.searchProductsByPrice(searchPrice, 50)
		.then(function(val){
        	...
        	alert("Error::processSearchByPrice: "+val);
    	});
}
...
document.getElementById("inputButtonByType").addEventListener('click',function(){
    processSearchByType(document.getElementById('inputByType').value);
});


document.getElementById("inputButtonByPrice").addEventListener('click',function(){
    processSearchByPrice(document.getElementById('inputByPrice').value);
});
